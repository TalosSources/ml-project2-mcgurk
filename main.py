...
def run_test(sample_taken=16):  
  ...
  reconstruction, last_hidden_state = autoencode_video(video[None, :sample_taken], audio[None, :sample_taken*AUDIO_SAMPLES_PER_FRAME, 0:1], model, device, SAMPLES_PER_PATCH=sample_taken)
  ...
  print(f"\n\n last hidden states size {last_hidden_state.size()}")

  return model, reconstruction, last_hidden_state, score, indices
